...
def test_ra1n_symbolic_transfer_certified_closure_doc():
    s = Path("docs/math/RA1N_SYMBOLIC_TRANSFER_CERTIFIED_CLOSURE.md").read_text()
    ...
    assert "RA1N_PROJECTION_ERROR_CERTIFIED_CLOSURE.md" in s
    assert "certified closed on the finite-basis packet-exhausted surface" in s

def test_ra1n_symbolic_transfer_certificates_run():
    gram = subprocess.run(
